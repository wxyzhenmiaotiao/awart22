...
export default class extends Component {
  ...
  backgone=(v)=>{
    localStorage.setItem("usersaddress",JSON.stringify(v))
        ...
        <div className="createGoods_box_body">
          {
            data.length!=0?data.map((v,i)=>{
              return <div key={i} className="addressBBixo" onClick={()=>this.backgone(v)}>
              <p>{v.name}</p>
            <p>{v.phone}</p>
          ...
          <div onClick={this.address} className="createGoods_box_body_line">
            <p className="createGoods_box_body_line_left">添加地址</p>
